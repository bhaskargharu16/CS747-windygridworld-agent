import numpy as np
class StochasticKingsWindyWorld(object):
    ...
    def take_step(self,action):
        ...
        self.destination[i,j,0,0] = max(i - 1 - wind[j], 0) #up
        self.destination[i,j,0,1] = j #up

        self.destination[i,j,1,0] = max(i - wind[j], 0) #right
        self.destination[i,j,1,1] = min(j + 1, self.width - 1) #right

        self.destination[i,j,2,0] = max(min(i + 1 - wind[j], self.height - 1), 0)#down
        self.destination[i,j,2,1] = j #down

        self.destination[i,j,3,0] = max(i - wind[j], 0) #left
        self.destination[i,j,3,1] = max(j - 1, 0) #left

        self.destination[i,j,4,0] =  max(i - 1 - wind[j], 0)#up right
        self.destination[i,j,4,1] =  min(j + 1, self.width - 1)#up right

        self.destination[i,j,5,0] =  max(min(i + 1 - wind[j],self.height - 1), 0)# down right
        self.destination[i,j,5,1] =  min(j + 1,self.width - 1)# down right

        self.destination[i,j,6,0] =  max(min(i + 1 - wind[j],self.height - 1), 0)#down left
        self.destination[i,j,6,1] =  max(j - 1, 0)#down left

        self.destination[i,j,7,0] =  max(i - 1 - wind[j], 0)#up left
...
class StochasticKingsSolver(object):
